# da-mysql.md — MySQL pro demo67: lokálně i v Kubernetes

Tento průvodce popisuje, jak přidat MySQL databázi do demo67 projektu — nejdřív lokálně přes Docker Compose, pak nasazení v Kubernetes s PVC.
...
spring:
  datasource:
    url: ${SPRING_DATASOURCE_URL:jdbc:mysql://localhost:3306/demo67db}
    username: ${SPRING_DATASOURCE_USERNAME:demo67admin}
    password: ${SPRING_DATASOURCE_PASSWORD:Demo67Pass1!}
...
- vytvoří databázi `demo67db`
- vytvoří uživatele `demo67admin`
...
docker exec -it demo67-mysql mysql -u demo67admin -pDemo67Pass1! demo67db
...
| id | name             | email            |
+----+------------------+------------------+
|  1 | Alice Nováková   | alice@demo67.com  |
|  2 | Bob Procházka    | bob@demo67.com    |
|  3 | Carol Dvořáková  | carol@demo67.com  |
+----+------------------+------------------+
```
...
```json
[
  {"id":1,"name":"Alice Nováková","email":"alice@demo67.com"},
  {"id":2,"name":"Bob Procházka","email":"bob@demo67.com"},
  {"id":3,"name":"Carol Dvořáková","email":"carol@demo67.com"}
]
```
...
```
__demo67-gitops/argocd-deploy-kustomization/
...
ArgoCD aplikace `mentors` sleduje `__demo67-gitops/argocd-deploy-kustomization/`.
Stačí commitnout a pushnout změny:

```bash
cd /home/claiv/demo67
git add __demo67-gitops/
...
kubectl apply -k __demo67-gitops/argocd-deploy-kustomization/mysql/

# Nebo celý stack
kubectl apply -k __demo67-gitops/argocd-deploy-kustomization/
```
...
kubectl exec -n mentors deploy/mentor-mysql -- \
  mysql -u demo67admin -pDemo67Pass1! demo67db -e "SELECT * FROM db_users;"
```
...
curl -X POST http://localhost:9111/api/db/add \
  ...
  -d '{"name": "David Kratochvíl", "email": "david@demo67.com"}'
```
...
curl -X PATCH http://localhost:9111/api/db/modify/1 \
  -H "Content-Type: application/json" \
  -d '{"name": "Alice Novotná", "email": "alice.nova@demo67.com"}'
```
...
kubectl exec -n mentors deploy/mentor-mysql -- \
  ...
  "ALTER USER 'demo67admin'@'%' IDENTIFIED BY 'Demo67Pass1!';"
```
